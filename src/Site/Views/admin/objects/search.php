...
<form class="filter btn-group">
    ...
    <div class="btn-group filter__item">
        ...
        <div class="dropdown-menu filter__item__list">
            ...
            <div class="filter__price-range" style="width: 500px">
                <input type="text" class="js-range-filter-price" name="filter-price" value=""
                       data-type="int"
                       data-min="0"
                       data-max="100000000"
                       data-from="<?=is_null($request->get("filter-price")) ? 0:(int)explode(";", $request->get("filter-price"))[0]?>"
                       data-to="<?=is_null($request->get("filter-price")) ? 100000000:(int)explode(";", $request->get("filter-price"))[1]?>"
                       data-grid="true"
                       data-step="10000"
                       data-postfix=" ₽"
                />
    ...
    <div class="btn-group filter__item">
        <button type="button" class="btn btn-outline-light filter__item__btn" data-bs-toggle="dropdown">
            <div class="d-flex align-items-center">
                <div class="flex-grow-1">
                    <div class="fs-14 fw-semibold">Дата публикации</div>
                </div>
                <div class="ms-3"><img src="/images/icons/arrow-down.svg"></div>
            </div>
        </button>
        <div class="dropdown-menu filter__item__list" style="width: 250px">
            <div>
                <button class="btn btn-primary w-100" type="submit">Применить</button>
            </div>
            <div class="m-3">
                Дата от:
                <input type="date" id="start" name="filter-date-from" value="<?=$request->get('filter-date-from')?>" class="form-control">
                Дата до:
                <input type="date" id="start" name="filter-date-to" value="<?=$request->get('filter-date-to')?>" class="form-control">
            </div>
        </div>
    </div>
