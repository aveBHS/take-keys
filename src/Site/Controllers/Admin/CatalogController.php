...
//            $filter = [ ["status", 0], ['isAd', 0] ];
        ...
        $objects = AdsObjectSellModel::select(
            $filter,
            [ ["created", "DESC"] ],
        ...
        if(is_null($objects) and $page > 0){
            $request->redirect("/panel/objects/catalog_sell");
            return;
        }
        ...
        $request->show(view("admin.objects.catalog", [
            "objects_count"     => $totalObjects,
            "objects"           => $objects,
            "title"             => "[Продажа архив] Каталог недвижимости",
            "elements_per_page" => env("elements_per_page") ?? 25,
            "current_page"      => $page,
            "origin_url"        => "/panel/objects/catalog_sell",
